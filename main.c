		...
		case Affichage:
			...
			if( (condition_quitter != 1) && Menu == 0){
				...
				for(int i=0; i<niveau[nbNiveau].sizeMap; i++){
					...
					if(niveau[nbNiveau].map[i].image_terrain != NULL) ecrisImage(niveau[nbNiveau].map[i].Xpt1, niveau[nbNiveau].map[i].Ypt1, niveau[nbNiveau].map[i].image_terrain->largeurImage, niveau[nbNiveau].map[i].image_terrain->hauteurImage, niveau[nbNiveau].map[i].image_terrain->donneesRGB);
				}
