...
static DonneesImageRGB *death_screen = NULL;
...
void gestionEvenement(EvenementGfx evenement)
{
	...
	block niveau[3];
 	niveau[0] = initialise_block2(niveau[0]);
	niveau[1] = initialise_block3(niveau[1]);
	niveau[2] = initialise_block4(niveau[2]);
	...
	switch (evenement)
	{
		case Initialisation:
			...
			death_screen = lisBMPRGB("../IMAGES/death_screen.bmp");
		...
		case Affichage:
			...
			if( (condition_quitter != 1) && Menu == 0){
				...
				if(niveau[index_niveau].image_exit != NULL) ecrisImage(niveau[index_niveau].sortie_Xpt1, niveau[index_niveau].sortie_Ypt1, niveau[index_niveau].image_exit->largeurImage, niveau[index_niveau].image_exit->hauteurImage, niveau[index_niveau].image_exit->donneesRGB);
				//affiche le terrain 
				couleurCourante(255, 0, 0);
				...
				if(termine_salle == 1){
					termine_salle = 0;
					for(int i=0; i<niveau[index_niveau].sizeMap; i++){
						libereDonneesImageRGB(&niveau[index_niveau].map[i].image_terrain);
					}
					index_niveau++;
					joueur = initialise_joueur(joueur);
					fantome = initialise_fantome(fantome);
			...
			if (etatBoutonSouris() == GaucheAppuye)
			{
				...
				if( (abscisseSouris() > largeurFenetre()*0.3 && abscisseSouris() < largeurFenetre()*0.7) && (condition_quitter == 1) ){
					if(ordonneeSouris() < hauteurFenetre()*0.625 && ordonneeSouris() > hauteurFenetre()*0.375){
						...
						if(index_niveau != nbNiveau) sauvegarde(parametres);
				...
				if(index_niveau == nbNiveau){
					if( abscisseSouris() > largeurFenetre()*0.3 && abscisseSouris() < largeurFenetre()*0.7){
						if(ordonneeSouris() > hauteurFenetre()*0.08 && ordonneeSouris() < hauteurFenetre()*0.15){
							index_niveau = 0;
							joueur = initialise_joueur(joueur);
							fantome = initialise_fantome(fantome);
							tpsJeu = 0;
...
void afficheTps(int tpsJeu){
	...
	rectangle(largeurFenetre()*0.3, hauteurFenetre()*0.08, largeurFenetre()*0.70, hauteurFenetre()*0.15);
	...
	//affichage d'une chaine "recommencer"
	epaisseurDeTrait(5);
	afficheChaine("RECOMMENCER", 35, largeurFenetre()*0.32, hauteurFenetre()*0.087);	
