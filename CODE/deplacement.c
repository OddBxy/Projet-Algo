...
int perdre_piege(Joueur joueur, block niveau, float vxJoueur, float gravite){
	for(int i=0; i<niveau.nbPieges; i++){
		if( (joueur.xPt2 >= niveau.pieges[i].xCoin2) && (joueur.xPt1 <= niveau.pieges[i].xCoin2)){
			if(joueur.yPt1 >= niveau.pieges[i].yCoin1 && joueur.yPt1 <= niveau.pieges[i].yCoin3 ){
				printf("test1\n");
				return 1;
			} 
		}
		else if( (joueur.xPt2 >= niveau.pieges[i].xCoin2) && (joueur.xPt1 <= niveau.pieges[i].xCoin2)){
			if(joueur.yPt1 <= niveau.pieges[i].yCoin1 && joueur.yPt2 >= niveau.pieges[i].yCoin1 ){
				printf("test2\n");
				return 1;
			} 
		}
		
		else if(joueur.yPt1 - gravite < niveau.pieges[i].yCoin3 && joueur.yPt1 - gravite > niveau.pieges[i].yCoin1  ){
			if( (joueur.xPt1 < niveau.pieges[i].xCoin2) && (joueur.xPt1 > niveau.pieges[i].xCoin1)) return 1;
			else if( (joueur.xPt2 < niveau.pieges[i].xCoin2) && (joueur.xPt2 > niveau.pieges[i].xCoin1))
			{
				printf("test3\n");
				return 1;
			} 
...
int sortie_salle(Joueur joueur, block Salle){
	...
	if( (joueur.xPt2 >= Salle.sortie_Xpt2 ) && (joueur.xPt1 <= Salle.sortie_Xpt2 )){
			if(joueur.yPt1 <= Salle.sortie_Ypt1 && joueur.yPt1 <= Salle.sortie_Ypt2 ) return 1; 
		}
		else if( (joueur.xPt2 >= Salle.sortie_Xpt2 ) && (joueur.xPt1 >= Salle.sortie_Xpt2 )){
			if(joueur.yPt1 <= Salle.sortie_Ypt1 && joueur.yPt1 <= Salle.sortie_Ypt2 ) return 1; 
		}
		
		else if(joueur.yPt1 < Salle.sortie_Ypt2){
			if( (joueur.xPt1 < Salle.sortie_Xpt2 ) && (joueur.xPt1 > Salle.sortie_Xpt1)) return 1;
			else if( (joueur.xPt2 < Salle.sortie_Xpt2 ) && (joueur.xPt2 > Salle.sortie_Xpt1)) return 1;
		}
